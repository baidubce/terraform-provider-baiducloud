...
* `auth_type` - (Optional, ForceNew) CFC Function Trigger auth type if source_type is http, support anonymous or iam
...
* `method` - (Optional, ForceNew) CFC Function Trigger method if source_type is http
...
* `resource_path` - (Optional, ForceNew) CFC Function Trigger resource path if source_type is http
