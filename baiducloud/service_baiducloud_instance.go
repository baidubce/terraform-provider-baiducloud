...
func (s *BccService) FlattenInstanceModelToMap(instances []api.InstanceModel) ([]map[string]interface{}, error) {
	...
	for _, inst := range instances {
		...
		result = append(result, map[string]interface{}{
			...
			"vpc_id":                   inst.VpcId,
			"ephemeral_disks":          s.FlattenCDSVolumeModelToMap(ephVolumes),
			"cds_disks":                s.FlattenCDSVolumeModelToMap(cdsVolumes),
			"system_disk":              s.FlattenCDSVolumeModelToMap(sysVolumes)[0],
