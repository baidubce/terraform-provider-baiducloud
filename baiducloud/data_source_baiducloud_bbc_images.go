...
output "images" {
  value = "${data.baiducloud_bbc_images.default.images}"
}
```
*/
package baiducloud
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"image_type": {
				...
				Optional:     true,
				ForceNew:     true,
				ValidateFunc: validation.StringInSlice([]string{"ALL", "System", "Custom", "Integration", "Sharing"}, false),
			},
			"name_regex": {
				Type:         schema.TypeString,
...
func dataSourceBaiduCloudBbcImagesRead(d *schema.ResourceData, meta interface{}) error {
	...
	imageMap := bbcService.FlattenBbcImageModelToMap(imageList)
