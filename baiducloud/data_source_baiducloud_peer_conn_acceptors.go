...
func dataSourceBaiduCloudPeerConnAcceptors() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceBaiduCloudPeerConnAcceptorsRead,
...
func dataSourceBaiduCloudPeerConnAcceptorsRead(d *schema.ResourceData, meta interface{}) error {
	...
	action := "Query Peer Conn Acceptors " + vpcID + "_" + peerConnID

	pcsResult := make([]map[string]interface{}, 0)

	if peerConnID != "" {
		pc, err := vpcService.GetPeerConnDetail(peerConnID, vpc.PEERCONN_ROLE_ACCEPTOR)
		if err != nil {
			return WrapErrorf(err, DefaultErrorMsg, "baiducloud_peer_conns", action, BCESDKGoERROR)
		}

		pcMap := flattenPeerConnAcceptorToMap(pc)
...
func flattenPeerConnAcceptorToMap(pc *vpc.PeerConn) map[string]interface{} {
